...
import (
	...
	"github.com/joe-zxh/lbbft"
	"github.com/joe-zxh/lbbft/client"
...
func newLBBFTServer(conf *options, replicaConfig *config.ReplicaConfig) *lbbftServer {
	...
	srv.initPayloadData()
	srv.lbbft = lbbft.New(replicaConfig, conf.TLS, time.Minute, time.Duration(conf.ViewTimeout)*time.Millisecond)
	srv.gorumsSrv.RegisterClientServer(srv)
	return srv
}

func (srv *lbbftServer) initPayloadData() { // 初始化一些常用的负载的大小
	srv.pldatas[0] = []byte(``)
...
func (srv *lbbftServer) getPayloadData(size int32) []byte {
...
func (srv *lbbftServer) Start(address string) error {
	lis, err := net.Listen("tcp", address)
	if err != nil {
		return err
	}

	err = srv.lbbft.Start() // 这里是hs的入口
	if err != nil {
		return err
	}
...
func (srv *lbbftServer) Stop() {
	srv.gorumsSrv.Stop()
	srv.cancel()
	srv.lbbft.Close()
}

func (srv *lbbftServer) ExecCommand(_ context.Context, cmd *client.Command, out func(*client.Empty, error)) {
...
func (srv *lbbftServer) onExec() {
	for cmds := range srv.lbbft.GetExec() {
